...
class LoanedBooksByUserListView(LoginRequiredMixin,generic.ListView):
    """
    Generic class-based view listing boooks on loan to current user.
    """
    model = BookInstance
...
class LoanedBooksAllListView(PermissionRequiredMixin,generic.ListView):
    """
    Generic class-based view listing all book on loan. Only visible to user with can_mark_returned permission.
    """
    model = BookInstance
...
def renew_book_librarian(request, pk):
    ...
    # If this is a GET or anything else, create the default form.
    else:
...
class AuthorCreate(CreateView):
    model = Author
    fields = '__all__'
    initial={'date_of_death':'12/10/2106',}
